...
[ExcludeFromCodeCoverage] // Taken from VFS
public static class StringExtensions
{
    /// <summary>
    /// Replaces two characters in-place.
    /// </summary>
    public static unsafe void ReplaceBackWithForwardSlashInPlace(this string text)
    {
        fixed (char* textPtr = text)
        {
            var span = new Span<char>(textPtr, text.Length);
            span.Replace('\\', '/', span);
        }
    }
    ...
    /// </summary>
    /// <param name="text">string for which to get hash code for.</param>
    /// <remarks>
    ...
    public static unsafe nuint GetNonRandomizedHashCodeLower(this ReadOnlySpan<char> text)
    {
        Span<char> lower_text = stackalloc char[text.Length];
        text.ToLowerInvariant(lower_text); // I have optimized version but I'll wait for .NET 8, which may bring even faster one
        
        fixed (char* src = &lower_text.GetPinnableReference())
        {
            int length = lower_text.Length; // Span has no guarantee of null terminator.
